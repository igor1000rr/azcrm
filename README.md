...
- **Vitest** + **Playwright** — unit, integration, e2e тесты
...
Чистые функции расчёта вынесены в `src/lib/finance/{commission-calc,work-hours,csv}.ts` и покрыты юнит-тестами (`tests/unit/`).
...
Три уровня — подробнее в [`tests/README.md`](./tests/README.md).

```bash
# Юнит + интеграционные (быстро, без БД)
npm run test
npm run test:watch        # режим разработки
npm run test:coverage     # покрытие в html

# E2E через Playwright (нужно поднятое приложение)
npm run db:push && npm run db:seed
npm run build && npm start &
npx playwright install chromium  # один раз
npm run test:e2e
npm run test:e2e:ui       # интерактивно
```

**Что покрыто:**
- **Unit** (`tests/unit/`) — `permissions` (~25 кейсов), `commission-calc`, `work-hours`, `csv`, `utils`.
- **Integration** (`tests/integration/`) — server actions с замоканным Prisma client: `addPayment` (8 кейсов с разными комбинациями `sequence` × `startFromN` × менеджеры), `upsertWorkLog/deleteWorkLog`, `markCommissionPaidOut/bulkMarkPaidOut`, `GET /api/leads/export`.
- **E2E** (`tests/e2e/`) — Playwright под Anna: логин/редирект, создание лида + платёж, открытие финансовых страниц, экспорт CSV возвращает 200 с BOM, видимость пунктов меню по роли.

CI workflow `.github/workflows/test.yml` гонит на каждый push/PR в `main` четыре джоба параллельно: `unit`, `typecheck` (`tsc --noEmit`), `lint` (`next lint`), `e2e` (Postgres service + миграции + сид + build + start + Playwright). Артефакт `playwright-report/` сохраняется на 7 дней при падении.
...
│   │   ├── finance/            # услуги, премии, ЗП, расходы
│   │   ├── work-calendar/      # рабочее время
│   │   └── settings/
...
│   │   ├── leads/export/       # CSV экспорт лидов
...
│   ├── audit.ts                # аудит-лог
│   ├── finance/                # commission-calc, work-hours, csv (юнит-тесты)
...
tests/
├── setup.ts                    # моки next/cache, next/navigation, next/headers
├── unit/                       # чистые функции
│   ├── permissions.test.ts
...
│   ├── add-payment.test.ts
│   ├── work-log.test.ts
│   ├── commission-mark.test.ts
│   └── leads-export.test.ts
└── e2e/                        # Playwright
    ├── fixtures.ts             # loginAs + adminPage fixture
    ├── auth.spec.ts
    ├── lead-flow.spec.ts
    ├── finance.spec.ts
    └── work-calendar.spec.ts

.github/workflows/test.yml      # CI: unit + typecheck + lint + e2e
