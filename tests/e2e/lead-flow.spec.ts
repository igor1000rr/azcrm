...
// URL карточки лида: /clients/<id>, где id != "new".
// Без этого условия regex /\/clients\/[a-z0-9]+/ совпадает с /clients/new
// и тест думает что попал на карточку, когда он ещё на форме создания.
const isLeadCardUrl = (url: URL) =>
  /^\/clients\/[a-z0-9]+/.test(url.pathname) && !url.pathname.endsWith('/new');
...
test.describe('Lead flow — Anna создаёт лида и регистрирует платежи', () => {
  test('создание лида через /clients/new', async ({ adminPage }) => {
    ...
    // Ждём перехода на карточку лида (НЕ /clients/new)
    await adminPage.waitForURL(isLeadCardUrl, { timeout: 15_000 });
    await adminPage.waitForLoadState('domcontentloaded');

    // Имя клиента появляется в нескольких местах (breadcrumb + h1) → .first()
  ...
  test('1-й платёж не создаёт комиссии (со 2-го по умолчанию)', async ({ adminPage }) => {
    ...
    // Открываем модалку нового платежа — кнопка содержит «Платёж» (с иконкой Plus)
    await adminPage.getByRole('button', { name: /платёж|платеж/i }).first().click();

    // В модалке Input type=number с autoFocus — самый надёжный селектор
    // (FormField label может не привязывать htmlFor через getByLabel)
    await adminPage.locator('input[type="number"]').last().fill('1000');
    await adminPage.getByRole('button', { name: /записать|сохранить/i }).first().click();

    // Проверяем что платёж появился в таблице
    await expect(adminPage.getByText(/\+\s*1[\s,]?000.*zł/).first()).toBeVisible({ timeout: 10_000 });
  });
